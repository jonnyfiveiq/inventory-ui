...
import type { ResourceDrift, DriftType } from '../api/client';
      ...
      <Flex alignItems={{ default: 'alignItemsCenter' }} spaceItems={{ default: 'spaceItemsSm' }}>
        ...
        <FlexItem>
          <span style={{ fontSize: '0.8rem', color: 'var(--pf-v5-global--Color--200)' }}>
            detected {fmt(event.detected_at)}
          </span>
        </FlexItem>
      </Flex>
...
export function DriftModal({ resourceId, resourceName, onClose }: DriftModalProps) {
...
export function DriftLabel({ driftTypes, onClick }: DriftLabelProps) {
  const hasDeleted  = driftTypes.includes('deleted');
  const hasRestored = driftTypes.includes('restored');
  const color: DriftColor = hasDeleted ? 'red' : hasRestored ? 'green' : 'orange';
  const icon = hasDeleted ? <TrashIcon /> : hasRestored ? <ArrowCircleUpIcon /> : <ExclamationTriangleIcon />;
  const text = hasDeleted ? 'Deleted' : hasRestored ? 'Restored' : 'Drift';
